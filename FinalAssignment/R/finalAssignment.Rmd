...
```{r}
length(which(mydata$click_bool == 1))
...
mydata <- mydata[which(mydata$srch_id < 2000),]
# mydata <- head(mydata,1000)
length((unique(mydata$srch_id)))
median((unique(mydata$srch_id)))
max(mydata$srch_id)
```
pairs plot
```{r}
...
aovpen=lm(price_usd~prop_review_score + consumer,data=mydata)
...
barplot(xtabs(booking_bool~prop_review_score,data=esoph)/totage)
