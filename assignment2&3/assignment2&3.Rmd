...
```
for P value at 0.022 We reject H0 and  we conclude that it is not likley that the sample is from the standard exponential distribution with λ = 0.035.
...
First of all, we will make the adjustments necessary for the three databases to be able to be compared. These adjustments are needed since the measurement procedures between both scientists were different.
```{r}
...
As it can be seen from the box plots, there is a significant difference between the measurements made in 1879 by Michelson in 1879 and 1882 and from Newcomb's measurements from 1882. The median values of the box plots are considerably different between the former and these two last. Furthermore, between the two measurements from 1882, Newcomb managed to obtain his results in a fairly narrower amplitude, which is maybe thanks to his methodology and/or equipment used.
...
Here, we will make use of both mean and median values of each data set. We have created a function to take into account this choice. The results are given right after.
```{r}
# for means
...
Intervals show that  all measurements are closely range within 299000 interval.
...
The speed of light closely resembles the measurements of the French physicists Fizeau and Foucault.
...
```
The sign test concludes that the true median is not equal to 32, therefore rejecting the null hypothesis that the median is smaller or equal to 32 days. 
...
# have to filter stuff here first 
...
Wilcox test shows that the null hypothesis that the delivery period is maximum of 70 days can not be rejected. Therefore it is plasuble to think that parts arrive after the maximum
***
...
The values for the t.test dropped from 0.05375 to 0.01278; for the Wilcox on, it increased from 0.006131 to 0.01278, and for the Kolmogorov-Smirnov, it remained the same.
...
Now, the values for the t.test dropped from 0.01278 to 0.008811; for the Wilcox on, it decreased from 0.01278 to 0.007291 (close to the first simulation - 0.006131), and for the Kolmogorov-Smirnov, it still remained the same.
***
***
...
The best way to represent this data is through separating it into two different data sets: one for "lemo" and another for "energy".
```{r}
...
```
then we perform Kruskal Wallis test on isofluorane,halothane and cyclopropane
```{r}
...
```
we conclude that all Kruskal-Wallis tests reject their null hypothesis that the concentration is the same under the different drugs.
this is different from the previous conlusion because the previous test assumed normality while this test did not assume normality.
