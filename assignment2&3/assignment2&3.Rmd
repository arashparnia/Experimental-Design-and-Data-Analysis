...
# Create table of means and medians
data_summary = matrix(c(mean(light1879), median(light1879),
                        mean(light1882), median(light1882),
                        mean(light), median(light)),
                      ncol=2,byrow=TRUE)
colnames(data_summary) = c("Mean", "Median")
rownames(data_summary) = c("light1879", "light1882", "light")
data_summary = as.table(data_summary)

# Create function to calculate confidence intervals
conf_int = function(x, type="mean", B=1000)
{
  if (!((type=="mean")|(type=="median"))){stop("Only 'mean' and 'median' are accepted as type.")}
  
  if (type=="mean"){t1=mean(x)}
    else if (type=="median"){t1=median(x)}
  ...
  tstar975=quantile(tstar,0.975)
  c(2*t1-tstar975,2*t1-tstar025)
}

# Determine confidence intervals

conf_int(light1879, type="mean")
conf_int(light1879, type="median")

conf_int(light1882, type="mean")
conf_int(light1882, type="median")

conf_int(light, type="mean")
conf_int(light, type="median")
...
par(mfrow=c(1,1))
boxplot(lemo, ylim=c(6.3,9))
boxplot(energy, ylim=c(6.3,9))

par(mfrow=c(2,2))
qqnorm(lemo$before,main="Normal Q-Q Plot lemo (before)",
       ylab="Quantiles lemo (before)")
qqnorm(lemo$after,main="Normal Q-Q Plot lemo (after)",
       ylab="Quantiles lemo (after)")
qqnorm(energy$before,main="Normal Q-Q Plot energy (before)",
       ylab="Quantiles energy (before)")
qqnorm(energy$after,main="Normal Q-Q Plot energy (after)",
       ylab="Quantiles energy (after)")
...
```{r}
run$diff = run$after - run$before
diff_lemo = subset(run,drink=="lemo")$diff
diff_energy = subset(run,drink=="energy")$diff

mean(run$diff);mean(diff_lemo);mean(diff_energy)
t.test(diff_lemo, diff_energy)
```
...
```{r}
par(mfrow=c(1,2))
qqnorm(diff_lemo,main="Normal Q-Q Plot time diff lemo",
       ylab="Quantiles time diff lemo")
qqnorm(diff_energy,main="Normal Q-Q Plot time diff energy",
       ylab="Quantiles time diff energy")
```
...
# Create boxplots of all 3 variables
...
dogsframeiso = data.frame(iso=as.vector(as.matrix(dogs)), group1=as.factor(rep(1:1,each=10)))
dogsframeiso[1:10,]

# Perform Kruskal Wallis test isofluorane
is.factor(dogsframeiso$group); is.numeric(dogsframeiso$group)
attach(dogsframeiso)
kruskal.test(iso,group1)

# Create a data frame for halothane
dogsframehalo = dogs[,-c(1,3)]
dogsframehalo = data.frame(halo=as.vector(as.matrix(dogs)), group2=as.factor(rep(1:3,each=10)))
dogsframehalo[12:21,]

# Perform Kruskal Wallis test halothane
is.factor(dogsframehalo$group); is.numeric(dogsframehalo$group)
attach(dogsframehalo)
kruskal.test(halo,group2)

# Create a data frame for cyclopropane
dogsframecyclo = dogsframecyclo[,-c(1,2)]
dogsframecyclo = data.frame(cyclo=as.vector(as.matrix(dogs)), group3=as.factor(rep(1:3,each=10)))
dogsframecyclo[22:30,]

# Perform Kruskal Wallis test isofluorane
is.factor(dogsframecyclo$group); is.numeric(dogsframecyclo$group)
attach(dogsframecyclo)
